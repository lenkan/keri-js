...
`test_vectors/support/keripy.ts` holds the shape of a generated fixture — the JSON the
...
`test:vector` reports through `scripts/test-reporter.ts` — `dot` plus a closing count — because a
thousand passing vector lines bury the handful that matter.

## Commands

```sh
npm run test            # Unit tests, no build needed
npm run test:vector     # CESR test vectors and KERIpy interop vectors
...
- `src/cesr/codes.ts`, `fixtures/cesr_test_vectors.json`, `fixtures/events/` and
  `fixtures/credentials/` are generated by `scripts/generate-*.py`. Do not edit them by hand. The
  loose `.cesr` files at the top of `fixtures/` are not generated: they are streams captured by hand
  from a running KERIpy, and there is no script that reproduces them. `fixtures/` is excluded from
  Biome — its JSON comes from Python's `json.dumps`, so formatting it would make every regeneration
  produce a diff. `codes.ts` stays formatted, because the generator's output already matches Biome.
