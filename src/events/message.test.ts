...
function randomNonTransferableKey(seed = "test"): { privateKey: Uint8Array; publicKey: string } {
  const encoder = new TextEncoder();
  const salt = blake3(encoder.encode(seed));
  const privateKey = ed25519.utils.randomSecretKey(salt);
  const publicKey = ed25519.getPublicKey(privateKey);
  return { privateKey, publicKey: cesr.encodeMatter({ code: MatterCode.Ed25519N, raw: publicKey }) };
}
...
test("Should create incept event", () => {
  const { publicKey } = randomKey("test");
...
test("Should serialize incept event without attachments", () => {
  const { publicKey } = randomKey("test");
  ...
  assert.strictEqual(message.serialize(), JSON.stringify(message.event) + "-VAA");
});
...
test("Should parse indexed signatures", () => {
  ...
  const message = KeyEventMessage.parse(
    [
      JSON.stringify(keri.incept({ k: [key0.publicKey, key1.publicKey] })),
      "-VAt",
      "-AAC",
      "AACo9sQ34vV5dvKDn9_XT7aqXjYrQUcIXsciy84D8LslsvJTYA5X0czckvo30fSgbleGeSYRjWoDuPIyizJpOPUP",
      "ABCOpOupeb-jKCZ5geaN-qDAE0I-nNb5QWxN0UonZdpjluAQMLgWzSErlP8dE2MqzL_ScIl885AjgHN_FLSN3xgD",
  ...
  assert.deepStrictEqual(message.attachments.sigs, [
    "AACo9sQ34vV5dvKDn9_XT7aqXjYrQUcIXsciy84D8LslsvJTYA5X0czckvo30fSgbleGeSYRjWoDuPIyizJpOPUP",
    "ABCOpOupeb-jKCZ5geaN-qDAE0I-nNb5QWxN0UonZdpjluAQMLgWzSErlP8dE2MqzL_ScIl885AjgHN_FLSN3xgD",
  ]);
});
...
test("Should parse receipt couples", () => {
  const key0 = randomKey("test0");
  const key1 = randomNonTransferableKey("test1");
  ...
  assert.deepStrictEqual(message.attachments.receipts, [
    {
