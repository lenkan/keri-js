...
describe("Key value store", () => {
  test("Should list inserted event", async () => {
    ...
    assert.partialDeepStrictEqual(result[0].body, event);
  });
  ...
  test("Should list events for identifier", async () => {
    ...
    assert.partialDeepStrictEqual(result0[0].body, event00);
    assert.partialDeepStrictEqual(result0[1].body, event01);

    const result1 = await store.list(event10.i);
    assert.equal(result1.length, 2);
    assert.partialDeepStrictEqual(result1[0].body, event10);
    assert.partialDeepStrictEqual(result1[1].body, event11);
  });
  ...
  test("Can insert and read seal", async () => {
    ...
    assert.equal(events.length, 1);
    assert.deepEqual(events[0].body, event);
  ...
  test("Should store credentials", async () => {
    ...
    assert.ok(stored);
    assert.deepStrictEqual(stored.body, credential);
  });
});
