...
export class ControllerEventStore {
  ...
  async save(event: Message<KeyEvent>) {
    switch (event.payload.t) {
      ...
      case "rot": {
        if (!event.payload.s) {
          throw new Error(`Event sequence number (s) is required for key event ${event.payload.t}(${event.payload.d})`);
        }

        const sn = event.payload.s.padStart(24, "0");
        await this.#db.set(
          `key_event.${event.payload.d}`,
          JSON.stringify({
            event: event.payload,
        ...
        await this.#db.set(`key_event_log.${event.payload.i}.${sn}`, event.payload.d);
        break;
      }
      case "rct": {
        await this.#db.set(
          `key_event_receipts.${event.payload.d}`,
          JSON.stringify(event.attachments.NonTransReceiptCouples || []),
        );
        break;
      }
      case "rpy":
        switch (event.payload.r) {
          case "/end/role/add": {
            const record = event.payload.a;
            if (
              record &&
    ...
    if (event.payload.v.startsWith("ACDC")) {
      await this.#db.set(
        `key_event.${event.payload.d}`,
        JSON.stringify({
          event: event.payload,
...
function reduce(state: KeyState, message: Message<KeyEvent>): KeyState {
  if (!message.payload.v.startsWith("KERI")) {
    return state;
  }
  ...
  switch (message.payload.t) {
    case "icp":
    case "dip": {
      const icp = message.payload as InceptEvent | DelegatedInceptEvent;
    ...
    case "ixn": {
      assertDefined(state);
      const ixn = message.payload as InteractEvent;
    ...
    default:
      throw new Error(`Unsupported event type: ${message.payload.t}`);
  }
}
