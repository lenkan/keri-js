import { Attachments, decodeMatter, encodeIndexer, Message, parse } from "cesr/__unstable__";
import {
...
import { getIndexedCode, serializeWitnessSignatures } from "./serializer.ts";
...
export class Controller {
  ...
  async createCredential(args: CreateCredentialArgs): Promise<CredentialEvent> {
    ...
    const state = await this.state(registry.payload.ii);

    const acdc = keri.credential({
      i: state.i,
      ri: registry.payload.i,
      s: args.schemaId,
      u: args.salt,
      a: {
        i: args.holder,
  ...
  private async submit(event: KeyEvent, signatures: string[]): Promise<string[]> {
    ...
    for (const wit of state.b) {
      ...
      for (const [other, receipt] of Object.entries(receipts)) {
        ...
        const message = new Message(receipt.payload, {
    ...
    if (credential.e) {
      for (const [, edge] of Object.entries(credential.e)) {
        if (typeof edge === "object" && "n" in edge && typeof edge.n === "string") {
          ...
          if (!source.payload.v.startsWith("ACDC")) {
            throw new Error(`Source for edge ${edge.n} is not a credential`);
          }

          await this.sendCredentialArficats(source.payload as CredentialEvent, recipient);

          if (
            (source.attachments.SealSourceCouples.length || source.attachments.SealSourceTriples.length) &&
            source.payload.i &&
            typeof source.payload.i === "string"
          ) {
            await this.forward(client, {
              message: source,
              recipient: recipient,
              sender: await this.state(source.payload.i),
              topic: "credential",
            });
          }
        }
  ...
  async grant(args: IpexGrantArgs): Promise<void> {
    ...
    if (!anchorSeal) {
      throw new Error(`No seal found for issuance ${iss.payload.d}`);
    }

    const anchor = await this.#store.get(anchorSeal.digest);

    if (!anchor) {
      throw new Error(`No anchor found for issuance ${iss.payload.d}`);
    }
      ...
      PathedMaterialCouples: [
        {
          ...
          attachments: {
            ...
            SealSourceTriples: [
              {
                prefix: iss.payload.i,
                snu: iss.payload.s,
                digest: iss.payload.d,
              },
            ],
          },
        },
        {
          ...
          attachments: {
            ...
            SealSourceCouples: [
              {
                digest: anchor.payload.d,
                snu: anchor.payload.s as string,
              },
            ],
          },
        },
        {
          ...
          attachments: {
            ...
            WitnessIdxSigs: serializeWitnessSignatures(
              anchor.attachments.NonTransReceiptCouples.map((r) => ({ backer: r.prefix, signature: r.sig })),
              state.b,
            ),
  ...
  async forward(client: Client, args: ForwardArgs): Promise<void> {
    if (client.role !== "mailbox" && client.role !== "witness") {
      ...
      const sigs = await this.sign(args.message.payload, args.sender.k);
      const message = new Message(args.message.payload, {
        grouped: false,
        ControllerIdxSigs: sigs,
      });
          ...
          TransIdxSigGroups: [
            {
              ...
              ControllerIdxSigs: await this.sign(args.message.payload, args.sender.k),
            },
          ],
        });
