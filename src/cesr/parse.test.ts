...
async function* chunk(filename: URL, size = 100): AsyncIterable<Uint8Array> {
  let index = 0;
...
describe(basename(import.meta.url), () => {
  ...
  describe("version handling", () => {
    ...
    test("should detect version 2 from genus counter", async () => {
      const input0 = await readFile(fixture("cesr_20.cesr"));
  ...
  describe("streaming behavior", () => {
    ...
    test("should parse chunked file stream", async () => {
      const data = chunk(fixture("geda.cesr"));
  ...
  describe("fixtures", () => {
    test("should parse alice fixture", { timeout: 100 }, async () => {
      const result = await collect(parse(createReadStream(fixture("alice.cesr"), {})));
    ...
    test("should parse witness fixture", { timeout: 100 }, async () => {
      const stream = createReadStream(fixture("witness.cesr"), {});
    ...
    test("should parse GEDA fixture", async () => {
      const stream = createReadStream(fixture("geda.cesr"), {});
    ...
    test("should parse credential fixture", async () => {
      const stream = createReadStream(fixture("credential.cesr"), {});
      const events = await collect(parse(stream));
    ...
    // A bare attachment declares no length, so it ends where its counters do. `kli vc export`
    // writes a credential's seal that way, and with `--chains` more than one of them in a row.
    test("should parse messages whose attachments carry no group counter", async () => {
      const bare = (message: Message) => decodeUtf8(message.raw) + encodeText(message.attachments.frames().slice(1));

      const first = new Message({ v: VersionString.KERI_LEGACY, t: "icp" }, { ControllerIdxSigs: [sig0] });
      const second = new Message({ v: VersionString.KERI_LEGACY, t: "ixn" }, { ControllerIdxSigs: [sig1] });

      const events = await collect(parse(bare(first) + bare(second)));

      assert.strictEqual(events.length, 2);
      assert.deepStrictEqual(events[0].attachments.ControllerIdxSigs, [sig0]);
    ...
    test("should parse mailbox fixture with TransIdxSigGroups", { timeout: 100 }, async () => {
      const result = await collect(parse(createReadStream(fixture("mailbox.cesr"), {})));
