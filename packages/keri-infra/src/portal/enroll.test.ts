...
import { createRouter } from "./portal-router.ts";

const URL_BASE = "http://portal.example";
...
async function makePortal() {
  ...
  const portal = await Portal.create({ storage, url: URL_BASE });
  return { portal, handler: createRouter(portal, storage) };
}
...
describe(basename(import.meta.url), () => {
  test("should enroll a controller and serve its oobi", async () => {
    const { portal, handler } = await makePortal();
    const user = makeUser();

    const enrolled = await handler(enrollRequest(user.kel, makeRpy(user, { eid: portal.aid })));
    ...
    // KERIpy's emit order: the KEL, the portal's location, the end-role authorization.
    ...
    assert.deepStrictEqual(endRole.body.a, { cid: user.aid, role: "mailbox", eid: portal.aid });
  ...
  test("should reject an rpy signed by the wrong key", async () => {
    ...
    const response = await handler(enrollRequest(user.kel, makeRpy(user, { eid: portal.aid, key: generateKeyPair() })));
    assert.strictEqual(response.status, 400);
  });

  test("should reject an rpy naming a different endpoint", async () => {
    const { handler } = await makePortal();
    const user = makeUser();

    const response = await handler(enrollRequest(user.kel, makeRpy(user, { eid: "ENOTTHISPORTAL" })));
    assert.strictEqual(response.status, 400);
  });
  ...
  test("should reject an rpy for a different role", async () => {
    ...
    const response = await handler(enrollRequest(user.kel, makeRpy(user, { eid: portal.aid, role: "agent" })));
    assert.strictEqual(response.status, 400);
  });
  ...
  test("should serve its own oobi at the bare path and its own aid", async () => {
    const { portal, handler } = await makePortal();

    for (const path of ["/oobi", `/oobi/${portal.aid}`]) {
      const response = await handler(new Request(`${URL_BASE}${path}`));
      assert.strictEqual(response.status, 200);
      assert.strictEqual(response.headers.get("Keri-Aid"), portal.aid);
      const messages = await Array.fromAsync(parse(await response.text()));
      assert.strictEqual(messages[0].body.i, portal.aid);
    }
  });

  // KERIpy's Poster sends to every advertised role, so a `mailbox` role on the
  // portal's own AID makes it deliver each message twice.
  test("should advertise the controller role for itself and never mailbox", async () => {
    const { portal, handler } = await makePortal();

    const response = await handler(new Request(`${URL_BASE}/oobi`));
    const messages = await Array.fromAsync(parse(await response.text()));
    const endRoles = messages
      .filter((message): message is Message<ReplyEventBody> => message.body.t === "rpy")
      .filter((message) => message.body.r === "/end/role/add")
      .map((message) => message.body.a);

    assert.deepStrictEqual(endRoles, [{ cid: portal.aid, role: "controller", eid: portal.aid }]);
  });

  // KERIpy composes request paths onto the advertised location, so it must be a
  // bare origin with no trailing slash.
  test("should advertise a bare origin", async () => {
    const storage = new SqliteControllerStorage(new NodeSqliteDatabase(new DatabaseSync(":memory:")));
    const portal = await Portal.create({ storage, url: `${URL_BASE}/` });
    const handler = createRouter(portal, storage);

    const response = await handler(new Request(`${URL_BASE}/oobi`));
    const messages = await Array.fromAsync(parse(await response.text()));
    const location = messages
      .filter((message): message is Message<ReplyEventBody> => message.body.t === "rpy")
      .find((message) => message.body.r === "/loc/scheme");

    assert.strictEqual(location?.body.a.url, URL_BASE);
  });
});
